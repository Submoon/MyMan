...
const eventFiles = glob.sync(pattern, {
...
commandFiles.forEach((file) => {
    const commandClass = require(`./commands/${file}`)
        .default as ICommandConstructor;
...
client.on("message", (message) => {
    ...
    for (const [id, user] of message.mentions.users) {
        AutoAnswer.where<AutoAnswer>({ userId: id, serverId: message.guild.id })
            .fetch()
            .then((ans) => {
                if (ans) {
                    message.channel.send(
                        `${user} said : ${ans.toJSON().autoanswer}`
                    );
                }
            })
            .catch((e) => {
                logger.error(
                    `Error while trying to access to autoanswers ${e.message}`
                );
            });
    }
